...
function GetFromBrowser($value_name, $old_value = '', $convert = true) {
    ...
    if (isset($HTTP_POST_VARS))
        $form_ary = array_merge($form_ary, $HTTP_POST_VARS);
    if (isset($HTTP_GET_VARS))
        $form_ary = array_merge($form_ary, $HTTP_GET_VARS);
    if (isset($_REQUEST))
        $form_ary = array_merge($form_ary, $_REQUEST);
    if (isset($_GET))
        $form_ary = array_merge($form_ary, $_GET);
...
function get_deffreq() {
    ...
    if (strpos($str, 'GPSINTERVAL_DEFAULT') === false)
        return str_pad(GPSINTERVAL_DEFAULT, 4, "0", STR_PAD_LEFT);

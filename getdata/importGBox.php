...
if($action == "import_GBox"){
    ...
    while (($objArr = fgetcsv($objCSV, 1000, ",")) !== FALSE) {
        if($objArr[0] != "Date" && $objArr[0] != ""){
            ...
            $sql_ck = "SELECT COUNT(*) AS num_row FROM data WHERE deviceid='".$device_id."' AND date=STR_TO_DATE('".$date."', '%Y-%M-%d') AND time='".$time."'";
            $res_ck = mysql_query($sql_ck, $link);
            $data_ck = mysql_fetch_array($res_ck);

            if($data_ck["num_row"] == 0){
                $sql = "INSERT INTO data(deviceid, date, time, latitude, longitude, altitude, speed, adc1, adc2) VALUES('".$device_id."', STR_TO_DATE('".$date."', '%Y-%M-%d'), '".$time."', '".$lat."', '".$lng."', '".$alt."', '".$speed."', '0', '0')";
                $res = mysql_query($sql, $link);
            }
        }
        $i++;

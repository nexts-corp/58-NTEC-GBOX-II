            ...
            <tbody>
                <tr>
                    <td>
                        ...
                        $exe1 = "SELECT  `test` , `name` , `route` , `date` , `time1` , `time2` ,`splevel` , `sptype` , `acclevel` , `acctype` ,  `turnlevel` ,  `turntype` ,  `total` ,  `distance`,  `v_score` ,  `a_score` ,  `t_score` FROM  `drivername` WHERE  `name` LIKE  '$DName' ";
                        $result1 = mysql_query($exe1)or die(mysql_error());
                        $num_rows = mysql_numrows($result1);
  
                        for ($i=1; $i<=$num_rows; $i++) {

                            list($test, $name , $route , $date , $time1 , $time2 , $splevel , $sptype , $acclevel ,$acctype ,$turnlevel , $turntype ,    $total ,  $distance, $vscore , $ascore , $tscore ) = mysql_fetch_row($result1);

...
<body>
    ...
    for ($k=1; $k<=$acc_cnt; $k++) {
        ...
        for ($i=$lo; $i<=$hi; $i++) {
            ...
            if ((($acc_stat[$sta_count-1]==4) OR ($acc_stat[$sta_count-1]==2)) AND (($acc_stat[$sta_count-2]==1) OR ($acc_stat[$sta_count-2]==3))) {
                $checkA[$k]=$checkA[$k]+1;;
            }
        ...
        if ((($accType[$k]=="AccNG") OR ($accType[$k]=="BrkNG")) AND ($StrType[$k]=="StrNG")) {
            $StrType[$k] = "Slalom";
            $typeNum01 = $typeNum01+1;

            if ($acc_vmax[$k]>=80) {
                $typeNum011 = $typeNum011+1;
            }
            else {
                $typeNum012 = $typeNum012+1;
            }
        }
        ...
        elseif (($speed0<=5) AND ($speed2<=10)) {
            $StrType[$k] = "Walking";
            $typeNum02 = $typeNum02+1;

            if (($accType[$k]=="AccNG") OR ($accType[$k]=="BrkNG")) {
                $typeNum021 = $typeNum021+1;
            }
            else {
                $typeNum022 = $typeNum022+1;
            }
        }
        ...
        elseif (($speed0>=$speed2) AND ($speed2>5) AND ($StrType[$k]=="StrNG") AND ($accType[$k]=="BrkG") AND (abs($intdelMax1[$k])<=10)) {
            $StrType[$k] = "CloseToSA";
            $typeNum34 = $typeNum34+1;
        }

        /*- 3.5 Sudden Close to - Slalom and Curve --- */
        elseif (($speed0>=$speed2) AND ($speed2>5) AND ($StrType[$k]=="StrNG") AND ($accType[$k]=="BrkG") AND (abs($intdelMax1[$k])>10)) {
            $StrType[$k] = "CloseToSAC";
            $typeNum35 = $typeNum35+1;
        }

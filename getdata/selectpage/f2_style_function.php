...
<body>
    ...
    if ($speed_avg100<>0) {
        $spdNrm = (100/$speed_avg100) * $speed_100;
    }
    if ($acc_avg100<>0) {
        $accNrm = (100/$acc_avg100) * $acc_100;
    }
    if ($turn_avg100<>0) {
        $turnNrm = (100/$turn_avg100) * $turn_100;
    }
    if ($zone_avg100<>0) {
        $zoneNrm = (100/$zone_avg100) * $zone_100;
    }
    if ($total_avg100<>0) {
        $totalNrm = (100/$total_avg100) * $total_100;
    }

    /* -------------------------------------------------------------------------- */
    if ($speed_max100!=0) {
        $speed_score = ($speed_100 / $speed_max100) * 100;
    }
    ...
    for ($i=1; $i<=$stp_point_cnt; $i++) {
        if (($SpeedMax>=81) AND ($SpeedMax<88)) {
            $vt[1] = $vt[1]+1;
        }
        elseif (($SpeedMax>=88) AND ($SpeedMax<96)) {
            $vt[2] = $vt[2]+1;
        }
        elseif (($SpeedMax>=96) AND ($SpeedMax<104)) {
            $vt[3] = $vt[3]+1;
        }
        elseif ($SpeedMax>=104) {
            $vt[4] = $vt[4]+1;
    ...
    switch ($vtmaxi) {
        ...
        case 4 : $tnp = "ระดับอันตรายมาก"; break;
    }

    $db = mysql_connect(DB_HOST, DB_USERNAME, DB_PASSWORD) or die("Error Connect to Database");
    $objDB = mysql_select_db(DB_NAME);

    $strSQL = "DELETE FROM  `drivername` WHERE  `name` = '$DName' AND  `route`  =  '$tripdir' AND  `date` =  '$Date1' AND  `time1` =  '$TimeBegin'  ;  ";
    ...
    $strSQL = "INSERT INTO `drivername` (`test` ,`name` ,`route` ,`date` ,`time1` ,`time2` ,`splevel` ,`sptype` ,`acclevel` ,`acctype` ,
                ...
                VALUES ('', '$DName', '$tripdir','$Date1','$TimeBegin','$TimeEnd','$sp_level','$sp_type','$acc_level','$acc_type',

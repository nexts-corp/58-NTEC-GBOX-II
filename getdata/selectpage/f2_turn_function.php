...
<body>
    ...
    $UTotal = 0; $curve_over = 0;

    $RA_score = 0; $RB_score = 0; $RC_score = 0; $RD_score = 0;
    $stp_point_cnt = 0;
    $jhk = 0; $sec0 = 0; $distance1 = 0; $str_point_cnt = 0; $distanceC = 0;
    $diff22 = 0; $maxtheta2 = 0; $CcountLB = 0; $LCB_score = 0; $LCB_weight = 0;
    $SCA_score = 0; $LD_score = 0; $LC_score = 0; $RCA_score = 0; $CcountLA = 0;
    $CcountRA = 0;
    ...
    for ($pp=1; $pp<=$num_rows; $pp++) {
        ...
        if ($pp>1) {
            ...
            if (($Ddir0!=0) AND ($startC==1)) {
                ...
                elseif (($int_diff2<=0) AND ($diffdiff>0)) {
                    ...
                    if ($DstopCnt==2) {
                        ...
                        $maxtheta0 = $direction;
                        $Ddir0 = 0;
                        $int_diff2 =0; $diffdiff=0; $D2cnt=$D2cnt+1;
                    }
                }
                else {
                    $DstopCnt = 0;
            ...
            if ((abs($Dstop)==1) AND ($startC==1) AND ($maxP==0)) {          /* Max delta Theta*/
                ...
                if (abs($maxDO) >= abs($maxD)) {
                    $maxD = $maxDO;
                }
                elseif (abs($maxDO) < abs($maxD)) {
                    $maxP = 1;
                }
            }

            /* ===================== U Turn Analytic ====================== */
            if (($delta_theta>140) OR ($int_dir>=100) AND ($startC==1) AND (($stopC==1) OR (abs($Dstop)==1))) {
                $stp_point_cnt = $stp_point_cnt+1;
                $delta_tt[$stp_point_cnt] = $int_dir;
            ...
            /* Turn analytic */
            if (($delta_theta>80) AND ($delta_theta<=140) AND ($startC==1) AND (($stopC==1) OR (abs($Dstop)==1))) {
                $stp_point_cnt = $stp_point_cnt+1;
                $delta_tt[$stp_point_cnt] = $int_dir;
                $stp_point[$stp_point_cnt] = "$str_pnt-$pp";
                $g_over[$stp_point_cnt] = $g_curve;
                ...
                if ($delta_distance <=45) {
                    ...
                    if ($speed_max<=20) {
                        $RT_fontA[$RcountA] = "#088A29";
                        $RA_weight = 1.0;
                    }
                    elseif (($speed_max>20) AND ($speed_max<=40)) {
                        $RT_fontA[$RcountA] = "#AEB404";
                        $RA_weight = 0.8;
                    }
                    elseif ($speed_max>40) {
                       $RT_fontA[$RcountA] = "#B40404";$RA_weight=0.6;
                    }
                ...
                elseif (($delta_distance >45) AND ($delta_distance <=100)) {
                    ...
                    if ($speed_max<=20) {
                        $RT_fontB[$RcountB] = "#088A29";
                        $RB_weight = 1.0;
                    }
                    elseif (($speed_max>20) AND ($speed_max<=40)) {
                        $RT_fontB[$RcountB] = "#AEB404";
                        $RB_weight = 0.8;
                    }
                    elseif ($speed_max>40) {
                        $RT_fontB[$RcountB] = "#B40404";
                        $RB_weight = 0.6;
                    }
                ...
                elseif (($delta_distance >100) AND ($delta_distance <=200)) {
                    ...
                    if ($speed_max<=20) {
                        $RT_fontC[$RcountC] = "#088A29";
                        $RC_weight = 1.0;
                    }
                    elseif (($speed_max>20) AND ($speed_max<=40)) {
                        $RT_fontC[$RcountC] = "#AEB404";
                        $RC_weight = 0.8;
                    }
                    elseif ($speed_max>40) {
                        $RT_fontC[$RcountC] = "#B40404";
                        $RC_weight = 0.6;
                    }
                ...
                elseif ($delta_distance >200) {
                    ...
                    if ($speed_max<=20) {
                        $RT_fontD[$RcountD] = "#088A29";
                        $RD_weight = 1.0;
                    }
                    elseif (($speed_max>20) AND ($speed_max<=40)) {
                        $RT_fontD[$RcountD] = "#AEB404";
                        $RD_weight = 0.8;
                    }
                    elseif ($speed_max>40) {
                        $RT_fontD[$RcountD] = "#B40404";
                        $RD_weight = 0.6;
                    }
            ...
            if (($delta_theta<-80) AND ($delta_theta>=-160) AND ($startC==1) AND (($stopC==1) OR (abs($Dstop)==1))) {
                ...
                if ($delta_distance <=45) {
                    ...
                    if ($speed_max<=20) {
                        $LT_fontA[$LcountA] = "#088A29";
                        $LB_weight = 1.0;
                    }
                    elseif (($speed_max>20) AND ($speed_max<=40)) {
                        $LT_fontA[$LcountA] = "#AEB404";
                        $LA_weight = 0.8;
                    }
                    elseif ($speed_max>40) {
                        $LT_fontA[$LcountA] = "#B40404";
                        $LA_weight = 0.6;
                    }
                ...
                elseif (($delta_distance >45) AND ($delta_distance <=100) ) {
                    ...
                    if ($speed_max<=20) {
                        $LT_fontB[$LcountB] = "#088A29";
                        $LB_weight = 1.0;
                    }
                    elseif (($speed_max>20) AND ($speed_max<=40)) {
                        $LT_fontB[$LcountB] = "#AEB404";
                        $LB_weight = 0.8;
                    }
                    elseif ($speed_max>40) {
                        $LT_fontB[$LcountB] = "#B40404";
                        $LB_weight = 0.6;
                    }
                ...
                elseif (($delta_distance >100) AND ($delta_distance <=200)) {
                    ...
                    if ($speed_max<=20) {
                        $LT_fontC[$LcountC] = "#088A29";
                        $LC_weight = 1.0;
                    }
                    elseif (($speed_max>20) AND ($speed_max<=40)) {
                        $LT_fontC[$LcountC] = "#AEB404";
                        $LC_weight = 0.8;
                    }
                    elseif ($speed_max>40) {
                        $LT_fontC[$LcountC] = "#B40404";
                        $LC_weight = 0.6;
                    }
                ...
                elseif ($delta_distance >200)  {
                    ...
                    if ($speed_max<=20) {
                        $LT_fontD[$LcountD] = "#088A29";
                        $LD_weight = 1.0;
                    }
                    elseif (($speed_max>20) AND ($speed_max<=40)) {
                        $LT_fontD[$LcountD] = "#AEB404";
                        $LD_weight = 0.8;
                    }
                    elseif ($speed_max>40) {
                        $LT_fontD[$LcountD] = "#B40404";
                        $LD_weight = 0.6;
                    }
            ...
            if ((($startC==1) AND ($stopC==1)) AND (abs($int_dir)>=6) AND (abs($delta_theta)<=80) AND (abs($maxD)>=10) AND ($D2cnt!=2)) {
                ...
                if ($delta_theta>0) {
                    $CcountRB = $CcountRB+1;
                    $CpointRB[$CcountRB] = $pp;

                    if ($speed_max<=30) {
                        $RC_fontB[$CcountRB] = "#088A29";
                        $RC_weight = 1.0;
                    }
                    elseif (($speed_max>45) AND ($speed_max<=60)) {
                        $RC_fontB[$CcountRB] = "#AEB404";
                        $RC_weight = 0.8;
                    }
                    elseif ($speed_max>60) {
                        $RC_fontB[$CcountRB] = "#B40404";
                        $RC_weight = 0.8;
                    }
                ...
                if ($delta_theta<0) {
                    $CcountLB = $CcountLB+1;
                    $CpointLB[$CcountLB] = $pp;

                    if ($speed_max<=30) {
                        $LC_fontB[$CcountLB] = "#088A29";
                        $RC_weight = 1.0;
                    }
                    elseif (($speed_max>45) AND ($speed_max<=60)) {
                        $LC_fontB[$CcountLB] = "#AEB404";
                        $LCB_weight = 0.8;
                    }
                    elseif ($speed_max>60) {
                        $LC_fontB[$CcountLB] = "#B40404";
                        $LCB_weight = 0.6;
                    }
            ...
            if (($startC==1) AND ($stopC==1) AND ($s_theta<15) AND (abs($maxD)>=15)  AND (abs($maxD)<=30))  {
                ...
                $CcountSA = $CcountSA+1;
                $CpointSA[$CcountSA] = $pp;

                if ($speed_max<=60) {
                    $SC_fontA[$CcountSA] = "#088A29";
                    $SCA_weight = 1.0;
                }
                elseif (($speed_max>60) AND ($speed_max<=80)) {
                    $SC_fontA[$CcountSA] = "#AEB404";
                    $SCA_weight = 0.8;
                }
                elseif ($speed_max>80) {
                    $SC_fontA[$CcountSA] = "#B40404";
                    $SCA_weight = 0.6;
                }
            ...
            elseif (($startC==1) AND ($stopC==1) AND ($s_theta<15) AND (abs($maxD)>30)) {
                ...
                $CcountSB = $CcountSB+1;
                $CpointSB[$CcountSB] = $pp;

                if ($speed_max<=60) {
                    $SC_fontB[$CcountSB] = "#088A29";
                    $SCB_weight = 1.0;
                }
                elseif (($speed_max>60) AND ($speed_max<=80)) {
                    $SC_fontB[$CcountSB] = "#AEB404";
                    $SCB_weight = 0.8;
                }
                elseif ($speed_max>80) {
                    $SC_fontB[$CcountSB] = "#B40404";
                    $SCB_weight = 0.6;
                }

                $SCB_score = $SCB_weight+$SCB_score;
                $startDis = 0;

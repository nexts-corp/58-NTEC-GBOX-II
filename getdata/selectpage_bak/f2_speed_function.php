...
<?php
    ...
    $osp[1]=0; $osp[2]=0; $osp[3]=0; $osp[4]=0; $over=0;   $sec0=0;
    ...
    $zero_cnt=0; $zero_stop=0; $zero_start=0;     $speed_str=0;
  ...
  for ($i=0; $i<=$num_rows; $i++) {
   ...
   elseif ($deviceD == "DLT02")  {$spd_unit=1;     $acc_limit=1.5;}
    ...
    if (($sec_del7 <=0) OR ($bb==0)) {$sec_del7=0;}
      $sec0 = $sec;
      $sec_zero[$bb]=$sec;
...
/* ==================================================== */
      ...
      if ($sec_del7 !=0)    {$av = (($v2-$v1) /$sec_del7 );}
      if ($k==0) {$av=0;}
      $av = round($av,2);
    ...
    $dis_sum = round(($dis_long/1000),4);                          /* Kilomater */
        ...
        elseif (($spd8>=81) AND ($speed_str==1)) {
          ...
          if ($spd8>$speed_max) { $speed_max=$spd8; $point_max = $bb; $lat_max0=$lat0; $lon_max0=$lon0;}
          ...
          if     ((abs($int_delT)>=7) AND ($take==0) AND ($take2==0)) { $spdT1=$sec0; $take=1; }
          elseif ((abs($int_delT)<7)  AND ($take==1) AND ($take2==0)) { $spdT3=$sec0-$spdT1; $take2=1;}

          if (abs($int_delT)>abs($intDmax) )                { $intDmax=$int_Dtheta[$bb]; $spdT2=$sec0;}

          $del_alt = $altitudE - $altitudE0;
          $alt_sumE = $alt_sumE + $del_alt;

          if     (abs($alt_sumE)>=abs($alt_max))  {$alt_max = $alt_sumE;}
        ...
        if (($speed_str==1) AND ($speed_stp==1) AND ($speed_sec>=40)) {
         ...
         $altMaxi[$over]= round($alt_sumE,2);
          ...
          if ($diff_dir[$over]<-180)     {$diff_dir[$over] = $diff_dir[$over] + 360;}     /* Right turn and cross Q4-Q1.  */
          elseif ($diff_dir[$over]>180)  {$diff_dir[$over] = $diff_dir[$over] - 360;}     /* Left turn and cross Q1-Q4.  */
          $diff_dir[$over] = round($diff_dir[$over],2);

          if ($speed_sec>=40) {$font_spd[$over] = "#DF0000";}
             elseif ($speed_sec<40) {$font_spd[$over] = "#000000";}
        ...
        elseif ($speed_max>=104)  {
          ...
          if     ($speed_max<81) { $colorp[$over]="m1.png";}
          elseif (($speed_max>=81) AND ($speed_max<88)) { $colorp[$over]="m2.png";}
          elseif (($speed_max>=88) AND ($speed_max<96)) { $colorp[$over]="m3.png";}
          elseif (($speed_max>=96) AND ($speed_max<104)) { $colorp[$over]="m4.png";}
          elseif ($speed_max>=104)  { $colorp[$over]="m5.png";}
    ...
    $alt_sumE =0;
        ...
        if (($spd8>=81) AND ($spd8<88)) { $g[1]=$g[1]+1; }

             elseif (($spd8>=88) AND ($spd8<96)) { $g[2]=$g[2]+1; }
             elseif (($spd8>=96) AND ($spd8<104)) { $g[3]=$g[3]+1; }
             elseif ($spd8>=104)  { $g[4]=$g[4]+1; }

/* ###############====== dowsy detection  ========================================*/

       if ($bb>=2) {$delta_direct = $direct[$bb] - $direct[$bb-1]; }
       ...
       if (($delta_speed<=1) AND ($dowsy_start==0)) {         /* 1 km/hr */
        ...
        elseif (($delta_speed<=1) AND ($dowsy_start==1)) {         /* 1 km/hr */
           ...
           if ($spd8>$speed_maxd) { $speed_maxd=$spd8; $point_maxd = $bb;}
           
           $int_speed = $int_speed + ($spd[$bb] - $spd[$bb-1]);
           
           $int_dirt = $int_dirt + $delta_direct;
            if ($int_dirt<-180)     {$int_dirt = $int_dirt + 360;}     /* Right turn and cross Q4-Q1.  */
            elseif ($int_dirt>180)  {$int_dirt = $int_dirt - 360;}     /* Left turn and cross Q1-Q4.  */
    ...
    if     (( $spd8<=10) AND ($lat1>=9.3128) AND ($lat1<=9.3528) AND ($lon1>=99.7238) AND ($lon1<=99.7638))  { $pointSea1 = $bb; }
    elseif (( $spd8<=10) AND ($lat1>=9.6885) AND ($lat1<=9.7285) AND ($lon1>=99.9641) AND ($lon1<=100.0041)) { $pointSea2 = $bb; }
...
/*   if ($bb>=1) {
   ...
   /*   $spd_dif = $spd[$bb] - $spd[$bb-1];
       ...
       if (($zero_start==0) AND ($zero_stop==0) AND ($spd_dif==0)  ) {
           $zero_start=1;
           $zp1 = $sec_zero[$bb];
           $zp_point1 = $bb-1;
           $spd_dif0 = $spd[$bb];
       ...
       elseif (($zero_start==1) AND ($zero_stop==0) AND ($spd_dif<>0))  {
           $zero_stop =1;
           $zp_point2 = $bb-1;
           $zp2 = $sec_zero[$bb];
       ...
       elseif (($zero_start==1) AND ($zero_stop==1)) {
...
/*         if ($sec_del7>=300) {
           ...
           $stop_bus_time[$stop_bus_cnt] = $sec_del7;
           $stop_bus_point1[$stop_bus_cnt] = $bb-1;
           $stop_bus_point2[$stop_bus_cnt] = $bb;

           $stop_bus_speed[$stop_bus_cnt] = $spd_dif;
 ...
 /* ========= Speed Type Detection============================================= */
   ...
   for ($bb=1; $bb<=$over; $bb++) {
    ...
    if ((abs($intDmax7[$bb])>=12) AND (abs($diff_dir[$bb])<=12) AND ($spdT7[$bb]<40) AND (abs($altMaxi[$bb])<20) ) {
       $spdType[$bb] = 1;   $type1=$type1+1;
    }

    elseif ((abs($intDmax7[$bb])>=12) AND (abs($diff_dir[$bb])<=12) AND ($spdT7[$bb]<40) AND (abs($altMaxi[$bb])>=20) ) {
       $spdType[$bb] = 4;   $type4=$type4+1;
    }

    elseif ((abs($intDmax7[$bb])>=12) AND (abs($diff_dir[$bb])>12) AND (abs($altMaxi[$bb])<20) ) {
       $spdType[$bb] = 2;   $type2=$type2+1;
    }
    elseif ((abs($intDmax7[$bb])>=12) AND (abs($diff_dir[$bb])>12) AND (abs($altMaxi[$bb])>=20) ) {
       $spdType[$bb] = 5;   $type5=$type5+1;
    }
    elseif ((abs($intDmax7[$bb])>=12) AND (abs($diff_dir[$bb])<=12) ) {
       $spdType[$bb] = 3;   $type3=$type3+1;
    }

    else {
            if      (abs($altMaxi[$bb])<20)  { $spdType[$bb] = 0; $type0=$type0+1;}
            elseif  (abs($altMaxi[$bb])>=20) { $spdType[$bb] = 6; $type6=$type6+1;}
          }
...
*/
  ...
  $mark = 0;  $start_zone = 0;  $stop_zone=0; $spd_zoning = 0; $spd_zone_cnt=0;
  $delta_spd_min = 999;
  $latOffset = 0.0015; $lonOffset = 0.0017;
  ...
  $int_delta=0;  $sec0=0;   $int_speed=0;  $int_dist=0;  $stop_bus_cnt=0;
  for ($i=1; $i<$num_rows; $i++) {

    $time0 = $time_i[$i];
    $lat0 = $lat_i[$i];
    $lon0 = $lon_i[$i];
    $spd0 = $speed_i[$i]*1.825;
    $dir0 = $dir_i[$i];
...
/* ---------------------------------------- */
  ...
  for ($n=0; $n<($dan_rows); $n++) {
     ...
     if (($lon1>=$lon_Rz[$n]) AND ($lon1<=$lon_Lz[$n]) AND ($lat1>=$lat_Rz[$n]) AND ($lat1<=$lat_Lz[$n])
          AND ( ($typez[$n]==1) OR ($typez[$n]==2) OR ($typez[$n]==3) OR ($typez[$n]==4) OR ($typez[$n]==5)) ){
...
/* ---------------------------------------- */
  if (($start_zone==1) AND ($stop_zone==0)) {
    ...
    if ($delta_spd<=$delta_spd_min) {$delta_spd_min=$delta_spd;}
   ...
   if (($start_zone==1) AND ($stop_zone==1)) {
      ...
      if ( ($delta_spd_min>=-20) AND ($spd0>=40) AND ($i>=15) ){
       ...
       if (($time>="14:00:00") AND ($time<="16:00:00")) {$primtime[$spd_zone_cnt]=1;}
       else                                             {$primtime[$spd_zone_cnt]=0;}
        
       $delta_spd_min = 999;
       
       }
       
     $start_zone = 0; $stop_zone = 0;    $int_delta=0;  $int_speed=0;
     $int_dist=0;
  }
...
<script language='JavaScript'>
     //parent.map.location.href = "f5_speed_heading.php"
</script >

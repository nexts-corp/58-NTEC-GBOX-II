...
elseif (($over_index>=600) AND ($over_index<700)) {
    ...
    $int_speed=0;  $delta_speed=0;   $int_acc =0;   $int_delT=0;
    for ($i=$lo; $i<=$hi ; $i++) {
      ...
      if ($sec_del!=0)
         { $acc = (($speedP[$i] - $speedP[$i-1])/$sec_del);  }      /* m/s2 */
      ...
      if ($sec_del!=0) { $accx = $spdA/$sec_del; }
  ...
  /*   $delta_dir = $dir_i[$i] - $dir_i[$i-1];
   ...
   /*   elseif ($int_delT>180)  {$int_delT = $int_delT - 360;}     /* Left turn and cross Q1-Q4.  */
    ...
    <?php echo " var options = { title: 'Point $over_index ($lo-$hi)' ,titleTextStyle: {color: 'blue', fontSize: 10} , ";   ?>
                      legend: {position: 'right', textStyle: {color: 'black', fontSize: 10}},

           <?php echo "   hAxis: {title: 'time (@second)',   count: 2, baselineColor: 'red'     " ; ?>
                             ,titleTextStyle:{color: 'black' , fontSize: 12}
                             },
                   vAxis: {title: 'Driving parameter',  baselineColor: 'green'
                             ,titleTextStyle:{color: 'black' , fontSize: 12}
                             },
...
<body onload="initialize()" onunload="GUnload()">
     ...
     <div id="chart_div" style="width: 450px; height: 240px;"> </div>
